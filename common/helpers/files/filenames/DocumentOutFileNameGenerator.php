...
class DocumentOutFileNameGenerator implements FileNameGeneratorInterface
{
    ...
    private function generateAppFileName($object, $params = [])
    {
        ...
        if ($object->company->short_name !== '') {
            $filename = 'Приложение'.($this->getOrdinalFileNumberApp($object) + $params['counter']).'_Вх.'.$new_date.'_'.$object->document_number.'_'.$object->companyWork->short_name.'_'.$object->document_theme;
        }
        else {
            $filename = 'Приложение'.($this->getOrdinalFileNumberApp($object) + $params['counter']).'_Вх.'.$new_date.'_'.$object->document_number.'_'.$object->companyWork->name.'_'.$object->document_theme;
        }
        $res = mb_ereg_replace('[ ]{1,}', '_', $filename);
        $res = mb_ereg_replace('[^а-яА-Я0-9._]{1}', '', $res);
        $res = StringFormatter::CutFilename($res);
